...
public class KeyboardBrass : AudioBaseType
{
    ...
    [Inspectable] public float frequency = 440f;
    [Inspectable] public float frequencyMod = 220f;
    ...
    private void CreateWaveOperators()
    {
        for (int i = 0; i < 128; i++)
        {
            int[] keyIndexes = new int[4];
            keyIndexes[0] = i;
            keyIndexes[1] = keyIndexes[0] + 19;
            keyIndexes[2] = keyIndexes[1] + 4;
            keyIndexes[3] = keyIndexes[2] + 24;
    ...
    public override void OnAudioRead(float[] buffer, int channels)
    {
        ...
        for (int i = 0; i < buffer.Length; i += channels)
        {
            for (int y = 0; y < 128; y++)
            {
                ...
                {
                    amp = keys[y] * t;
					float freq = AudioUtility.GetPianoFrequency(y, frequency);
					freq = (float)Math.Sin(2 * Math.PI * timecount * freq / 44100);
					freq += AudioUtility.GetPianoFrequency(y, frequencyMod);
					x += (float)Math.Sin(2 * Math.PI * timecount * freq / 44100) * 0.25f;
                }
            }
