...
public class KeyboardWurley : AudioBaseType
{
	long timer = 0;
	...
	void SetEnvelopes(int keyIndex)
	{
        ...
        envelope3[keyIndex].DecayRate = 0.25f * sampleRate;
        ...
        envelope4[keyIndex].DecayRate = 0.15f * sampleRate;
	...
	float GetModulatedWave(int keyIndex, long time)
	{
		...
		x = (float)Math.Sin(2 * Math.PI * time * (baseFreq*4.0) / 44100 + x);
		x *= envelope2[keyIndex].Process();

		y = (float)Math.Sin(2 * Math.PI * time * baseFreq / 44100);
		y = (float)Math.Sin(2 * Math.PI * time * -510.0 / 44100 + y);
