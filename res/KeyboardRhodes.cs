...
public class KeyboardRhodes : AudioBaseType
{
	long timer = 0;
	...
	void SetEnvelopes(int keyIndex)
	{
		...
		envelope1[keyIndex].AttackRate = 0.001f * sampleRate;
        envelope1[keyIndex].DecayRate = 1.50f * sampleRate;
        envelope1[keyIndex].SustainLevel = 0;
        envelope1[keyIndex].ReleaseRate = 0.04f * sampleRate;
        envelope1[keyIndex].State = Envelope.EnvelopeState.Attack;

		envelope2[keyIndex].AttackRate = 0.001f * sampleRate;
        envelope2[keyIndex].DecayRate = 1.50f * sampleRate;
        envelope2[keyIndex].SustainLevel = 0;
        envelope2[keyIndex].ReleaseRate = 0.04f * sampleRate;
        envelope2[keyIndex].State = Envelope.EnvelopeState.Attack;

		envelope3[keyIndex].AttackRate = 0.001f * sampleRate;
        envelope3[keyIndex].DecayRate = 1.00f * sampleRate;
        envelope3[keyIndex].SustainLevel = 0;
        envelope3[keyIndex].ReleaseRate = 0.04f * sampleRate;
        envelope3[keyIndex].State = Envelope.EnvelopeState.Attack;

		envelope4[keyIndex].AttackRate = 0.001f * sampleRate;
        envelope4[keyIndex].DecayRate = 0.25f * sampleRate;
        envelope4[keyIndex].SustainLevel = 0;
        envelope4[keyIndex].ReleaseRate = 0.04f * sampleRate;
	...
	float GetModulatedWave(int keyIndex, long time)
	{
		...
		x = (float)Math.Sin(2 * Math.PI * time * (baseFreq*1.0) / 44100 + x);
		x *= envelope1[keyIndex].Process();

		x = (float)Math.Sin(2 * Math.PI * time * (baseFreq*0.5) / 44100 + x);
		x *= envelope2[keyIndex].Process();

		y = (float)Math.Sin(2 * Math.PI * time * baseFreq / 44100);
		y = (float)Math.Sin(2 * Math.PI * time * (baseFreq*1.0) / 44100 + y);
		y *= envelope3[keyIndex].Process();

		y = (float)Math.Sin(2 * Math.PI * time * (baseFreq*15.0) / 44100 + y);
